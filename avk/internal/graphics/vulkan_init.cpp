...
int vulkan_on_window_resize()
{
	...
	vkDestroyPipelineLayout(device, bar_graph_pipeline_layout, nullptr);
	vkDestroyRenderPass(device, renderpass, nullptr);
	for (auto v : swapchain_image_views)
		vkDestroyImageView(device, v, nullptr);

	bool recreate_semaphores = false;
	VkResult result;
	uint32_t k;
	constexpr auto base_counter = __COUNTER__;

	{
		...
		vkDestroySwapchainKHR(device, info.oldSwapchain, nullptr);
		...
		framebuffers.resize(k);
		command_buffers.resize(k);
		if (begin_render_semaphore.size() != k)
		{
			recreate_semaphores = true;
			for (auto s : begin_render_semaphore)
				vkDestroySemaphore(device, s, nullptr);
			for (auto s : end_render_semaphore)
				vkDestroySemaphore(device, s, nullptr);
			begin_render_semaphore.resize(k);
			end_render_semaphore.resize(k);
		}
